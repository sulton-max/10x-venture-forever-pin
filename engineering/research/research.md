...
- `error-ordering/error-ordering.md` — cited answer to the open question in [validation](../planning/validation.md): resolve+authorize (`404`/`403`) vs payload validation (`400`/`422`). RFC 9110 / AIP-211 / CWE-203-204 / OWASP + Rails·DRF·Laravel·ASP.NET Core source-verified sequencing, rigour-tagged `[S]`/`[F]`/`[G]`/`[O]`.
