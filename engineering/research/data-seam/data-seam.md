...
Needs a throwaway Postgres. Default connection `Host=localhost;Port=5432;Database=foreverpin_dataseam;Username=postgres;Password=postgres`; override via `argv[0]` or the `DATASEAM_DB` env var.

```bash
createdb foreverpin_dataseam           # or: psql -c 'create database foreverpin_dataseam'
dotnet run
```
