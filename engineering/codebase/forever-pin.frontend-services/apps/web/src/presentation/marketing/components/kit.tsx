...
export function Logo({ className = "" }: { className?: string }) {
  return (
    <span className={`inline-flex shrink-0 items-center gap-2 font-bold ${className}`}>
      ...
      <span className="whitespace-nowrap text-lg tracking-tight">ForeverPin</span>
    </span>
  );
}
    ...
    <Table radius="2xl" density="roomy" className="min-w-[34rem]">
      <TableHead headVariant="plain">
        <TableRow>
          ...
          <TableHeaderCell className="text-primary">ForeverPin</TableHeaderCell>
    ...
    >
      {items.map((item, i) => (
        <Accordion.Item key={item.q} value={String(i)} className="border-b-0">
          ...
          <Accordion.Content>
            <div className="-mt-1 px-5 pb-5 leading-relaxed text-muted-foreground">{item.a}</div>
          </Accordion.Content>
        </Accordion.Item>
      ))}
    </Accordion>
  ...
  return (
    ...
    >
      <div className="flex justify-center">
        <StaticQrPreview value="https://foreverpin.com/demo" foreground="#18181b" background="#ffffff" size={200} />
      </div>
      <VStack gap="3">
  ...
  return (
    <div className="relative">
      ...
      <div className="relative rounded-3xl border border-border bg-card p-6 shadow-xl shadow-primary/5">
        <StaticQrPreview value="https://foreverpin.com/menu" foreground="#6d28d9" background="#ffffff" size={220} />
        <div className="mt-4 flex items-center justify-between gap-2 text-xs">
          <span className="font-medium text-muted-foreground">foreverpin.com/menu</span>
