...
using ForeverPin.Tests.Integration.Harness;

namespace ForeverPin.Tests.Integration;

/// <summary>Integration tests for the subscription persistence path.</summary>
public class SubscriptionRepositoryTests(ForeverPinTestDb db) : RepositoryTestBase(db)
{
    private static SubscriptionEntity NewSub(
        ...
        string sub = "sub_1",
        string cus = "cus_1",
        DateTimeOffset? periodEnd = null) => new()
    {
        Id = Guid.NewGuid(),
        UserId = user,
        Plan = plan,
    ...
    public async Task Upsert_inserts_when_no_row_then_get_by_user_returns_it()
    {
        ...
        await new SubscriptionRepository(NewContext()).UpsertByUserAsync(
            NewSub(user, Plan.Pro, SubscriptionStatus.Active),
            default);
    ...
    public async Task Upsert_overwrites_existing_row_keeping_one_per_user()
    {
        ...
        await new SubscriptionRepository(NewContext()).UpsertByUserAsync(
            NewSub(user, Plan.Solo, SubscriptionStatus.Active, sub: "sub_A"),
            default);
        // Re-subscribe / plan change → same user, overwrite (single-row policy, status flips, new sub id).
        await new SubscriptionRepository(NewContext()).UpsertByUserAsync(
            NewSub(user, Plan.Pro, SubscriptionStatus.Active, sub: "sub_B"),
            default);
    ...
    {
        await new SubscriptionRepository(NewContext()).UpsertByUserAsync(
            NewSub(Guid.NewGuid(), Plan.Solo, SubscriptionStatus.Active, sub: "sub_lookup"),
            default);

        var found = await new SubscriptionRepository(NewContext()).GetByStripeSubscriptionIdAsync(
            "sub_lookup",
            default);
        Assert.NotNull(found);
        Assert.Equal("sub_lookup", found!.StripeSubscriptionId);

        Assert.Null(await new SubscriptionRepository(NewContext()).GetByStripeSubscriptionIdAsync(
            "sub_missing",
            default));
    }
    ...
    public async Task Status_transition_active_to_canceled_persists()
    {
        var user = Guid.NewGuid();
        await new SubscriptionRepository(NewContext()).UpsertByUserAsync(
            NewSub(user, Plan.Pro, SubscriptionStatus.Active, sub: "sub_X"),
            default);

        // Mirror customer.subscription.deleted: same sub id, status → Canceled.
        await new SubscriptionRepository(NewContext()).UpsertByUserAsync(
            NewSub(user, Plan.Pro, SubscriptionStatus.Canceled, sub: "sub_X"),
            default);
